...
- 订单管理使用独立 `OrderManagementPage`，只调用平台级 `admin.orders.list`。用户侧
  `order.history` 受当前用户会话约束且只返回已完成订单，管理员端禁止复用或伪造
  用户身份。
- 订单列表由服务端一次性批量关联订单、用户、站点和充电桩，避免逐行请求造成 N+1、
  半行旧数据或错误归属。搜索与分页也全部由服务端执行，客户端只校验返回结果。
- `Order` DTO 对状态、支付、金额类型和四个生命周期时间做交叉校验。充电中金额仅为
  `generatedAt` 时刻的服务端预估；待支付/已完成才使用最终金额，只有已完成计入营收。
- 页面用实体角色保存 `orderId`，当前页排序后仍能打开正确详情。详情将用户、设备、
  计费和生命周期拆分展示，避免把所有字段挤入宽表导致低分辨率遮挡。
- 当前订单页面只读。强制停止、代结算、改价、删除与退款需要成员 A 先定义权限、
  审计原因、幂等、余额补偿、设备释放和营收冲正规则，客户端不自行扩权。
- `admin.orders.list` 是等待 A 实现的客户端先行契约；可复用的状态和实体字段保持与
  A 当前订单服务一致，新增的支付/金额标签及平台汇总已在公开协议逐项标明。
