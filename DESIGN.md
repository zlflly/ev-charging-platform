...
站点列表突出名称、距离、电价和真实空闲数；站点详情把站名、地址、状态、电价与大号空闲数合并在一张浅蓝身份卡中。充电桩详情使用真实透明设备渲染图，并把桩编号、状态、功率、枪型、价格和站点位置组织成完整设备信息流，预约与导航固定在底部。路线页让地图占据大部分视口，底部只保留终点、时间、距离和开始导航操作。

### Charge Gauge

完整环形仪表只表达真实订单进度；蓝绿渐变对应充电进程，中心显示百分比和充电状态。左右波形分别读取服务端功率与电量趋势数组；没有趋势数据时显示空基线，不制造曲线。四张彩色指标卡固定为电量蓝、功率绿、时长蓝和费用橙，并保持数值与单位单行显示。
